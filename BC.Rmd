...
```{r}
M1 = glm(avg ~ round*as.factor(size)-1, data=amt[amt$round<5,])
anova(M0,M1, test='LRT')
```
We find that all terms are now significant, and adapt `M1` as the final model.
```{r}
summary(M1)
```
...
b2.est = t(b2)%*%coef(M1)
b2.se  = sqrt(t(b2)%*%vcov(M1)%*%b2)

b4.est = t(b4)%*%coef(M1)
b4.se  = sqrt(t(b4)%*%vcov(M1)%*%b4)

b8.est = t(b8)%*%coef(M1)
b8.se  = sqrt(t(b8)%*%vcov(M1)%*%b8)
...
pred = predict(M1,newdata = tmp1, se.fit = TRUE)
